...
var Model = module.exports = (function ModelConstructor(name, schema, initialize) {
  ...
  // FIXME: Another bastardization of constructors. I need to write my own "[constructor Constructor]" which inherits from Function.
  eval("var Constructor = function " + name + "(data) { _(this).extend(this.constructor._schema);  _(this).extend(data); var initialize = this.constructor._initialize; if (initialize) initialize(); }")
  ...
  .method(function save() {
    ...
    if (this._events.add) this._events.add()
  })
  ...
  .method(function get(name) {
    ...
    this[name] = value
    if (this._events.change) this._events.change()
    return this[name]
  })
  ...
  /**
   *
   * Getters/settersApply a callback to an event.
   *
   */
  .method(function on(events, callback, context) {
    var that = this
    events.split(' ').forEach(function (event) {
      that._events[event] = callback
      if (context) that._events[event].bind(context)
    })
  })
