...
function Model(name, schema, initialize) {
  ...
  if (!~['production', 'test'].indexOf(this.env)) global[name] = Constructor
  ...
  Constructor.all = function (callback) {
    if (callback instanceof Function) {
      ...
      return Constructor.db.find().stream().pipe(_.mapStream(function (data, callback) {
        callback(null, new Constructor(data))
      }))
    }
  }

  Constructor.union = function () {
    return Constructor.db.union.apply(Constructor.db, arguments)
  }
