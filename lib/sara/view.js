  ...
  if (options.render) this.render = function (context, callback) {
    ...
    if (IS_SERVER) {
      ...
      if (options.prerender instanceof Function || options.prerender === false) {
        ...
        for (var i in Sara.resources) {
          var resource = Sara.resources[i]
          readable.push('<script type="text/json" data-model="' + resource.name.toLowerCase() + '">' + resource.jsonContent + '</script>')
        }

        // for (var id in Sara.templates) {
        //   if (id !== 'layout') readable.push('<script type="text/template" data-template="' + id + '">' + Sara.templates[id].templateContent.encoded + '</script>')
        // }

        readable.push('<meta name="saraClientId" content="'+ id + '">' + '<script src="/index.js" type="text/javascript"></script>')
        readable.push(Sara.layout.toString())
        readable.push(null)

        // Stream the prerender function
        // if (options.prerender instanceof Function) options.prerender(context)
      } else {
