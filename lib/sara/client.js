...
module.exports = function setupClient(Sara, callback) {
  ...
  function ready() {
    ...
    window.onpopstate = function () {
      Sara.paths[window.location.pathname](readable, writable)
    }

    // A client-side router
    document.addEventListener('click', function (event) {
      if (event.target.host === window.location.host && !event.target.getAttribute('data-bypass') && Sara.visit(event.target.pathname) !== undefined) {
        event.preventDefault()
        window.history.pushState(null, event.target.textContent, event.target.href)
        Sara.visit(event.target.pathname)(readable, writable, window)
      }
    })
