...
module.exports = function Client(app) {
  ...
  socket.onmessage = function (json) {
    ...
    if (e.refresh) {
      console.warn('Client DOM has become desynchronized from the server, reloading to prevent errors.')
      window.location.reload()
    } else if (e.modelEvent) {

    } else if (e.collectionEvent) {
      // Format:
      //
      // {
      //   collectionEvent: {
      //     model: 'Todo'
      //   , event: 'add'
      //   , data: { title: 'foo', completed: false }
      //   , id: 1
      //   , set: { title: 'foo' }
      //   }
      // }

      var Constructor = app.resources[e.collectionEvent.model]

      switch (e.collectionEvent.event) {
        case 'add':
        new Constructor(e.collectionEvent.data).save()
        break
        case 'remove':
        Constructor.find(e.collectionEvent.id).destroy()
        break
        case 'changeAny':
        Constructor.find(e.collectionEvent.id).set(_.keys(e.collectionEvent.set)[0], e.collectionEvent.set[0])
        break
      }
    }
  }
