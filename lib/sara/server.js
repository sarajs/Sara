...
var Server = module.exports = (function ServerConstructor(app) { 
  ...
  // Compile Sara
  bundle.add(path.join(__dirname, '../sara'))
  bundle.add(path.join(__dirname, '/model'))
  bundle.add(path.join(__dirname, '/controller'))
  
  // Compile the app
  bundle.add(require.main.filename)
  
  // Prevent sara/server form loading
  bundle.ignore('./sara/server')
