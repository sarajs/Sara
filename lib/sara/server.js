...
var Server = module.exports = (function ServerConstructor(app) { 
  ...
  app.adapter = function (config) {
    config.bundle(app)
  }
  
  // Compile Sara
  app.bundle.add(path.join(__dirname, '../sara'))
  app.bundle.add(path.join(__dirname, '/model'))
  app.bundle.add(path.join(__dirname, '/client'))
  
  // Compile the app
  app.bundle.add(require.main.filename)
  
  // Prevent sara/server form loading
  app.bundle.ignore('./sara/server')
