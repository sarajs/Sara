  ...
  app.bundle.bundle(function (err, src) {
    ...
    socket.on('connection', function (socket) {
      socket.on('message', function (json) {
        ...
        new Sara.Event({
          type: e.type
        , target: nodeFromSaraId(e.target.saraId)
        , bubbles: true
        , cancelable: true
        , targetValue: e.target.value
        , srcElement: ~['keydown'].indexOf(e.type) ? document.documentElement : nodeFromSaraId(e.srcElement.saraId)
        })

        console.log(document.querySelector('ol').childNodes.length)
      })
    })

    function nodeFromSaraId(saraId) {
