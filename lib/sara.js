...
var Sara = module.exports = (function Sara(options) {
  ...
  // Defaults
  if (IS_SERVER) this.root = _.dirnameFromStackIndex(2)
...
.add('Utils', _)
.add(function template(filepath) {
  if (IS_SERVER) {
    var fs = require('fs')
      , path = require('path')

    return fs.readFileSync(path.resolve(_.dirnameFromStackIndex(2), filepath))
  } else if (IS_CLIENT) {
    // foo
  }
})
