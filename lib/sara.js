...
function Sara() {
  ...
  if (!(this instanceof Sara)) return new Sara(options)
...
Sara.prototype.init = function (options, fn) {
  if (IS_SERVER) setupServer(this, callback)
  else setupClient(this, callback)

  function callback() {
    if (fn !== undefined) fn.bind(this)()
  }
...
Sara.prototype.route = function (request) {
  ...
  for (var path in this._routes[method]) {
    var match = url.match(new RegExp(path.replace(/:[^\s\/.]+/g, '(.+)')))
    if (match && url === match[0]) return this._routes[method][path].bind(this, match[1])
  }
