...
var Sara = module.exports = (function Sara(options) {
  ...
  this.routes['/' + name + 's/'] = { template: name + 's/new.html', method: 'POST' }
  this.routes['/' + name + 's/' + 'new'] = { template: name + 's/new.html', method: 'GET' }
  this.routes['/' + name + 's/' + ':id'] = { resource: Constructor, action: 'find', template: name + 's/show.html', method: 'GET' }
  this.routes['/' + name + 's'] = { resource: Constructor, action: 'all', template: name + 's/index.html', method: 'GET' }
  this.routes['/' + name + 's/' + ':id/' + 'edit'] = { resource: Constructor, action: 'find', template: name + 's/edit.html', method: 'GET' }
  this.routes['/' + name + 's/' + ':id/' + 'delete'] = { resource: Constructor, action: 'find', template: name + 's/delete.html', method: 'DELETE' }
