 ...
 * @param {String} - msg - A string to log to the console
 * @returns {Sara}
 */
Sara.prototype.log = function (msg, prefix) {
  prefix = prefix !== false ? red + '[sara] ' : prefix ? prefix : ''
  if (!global.autoPrerender && !~['production', 'test'].indexOf(this.env)) console.log(prefix + msg + normal)
  return this
}

Sara.prototype.ignore = function (id) {
...
Sara.prototype.template = function (id, filepath, index) {
  if (IS_SERVER) {
    ...
    var templ = {
      content: contents
    , compiled: Handlebars.compile(contents.toString())
    , id: id
    , toString: function () {
        return this.content.toString()
      }
    , encoded: _.htmlEncode(contents.toString())
    }
...
Sara.prototype.createRoutes = function (method, paths, action) {
  ...
  if (_.typeOf(paths) == 'array') {
    paths.forEach(function (path) {
      ...
      var routeKeys = []
        , routePattern = pathtoRegexp(path, routeKeys)
...
Sara.prototype.minify = function (bool) {
  this.minified = bool
  return this
}
...
Sara.prototype.static = function (directory) {
  if (IS_SERVER) {
    ...
    _(files).forEach(function (file) {
      ...
      _(this.filters[path.extname(filepath)]).forEach(function (filter) {
        contents = filter(contents.toString())
      })
      
      this.assets['/' + file] = { filepath: filepath, contents: contents, etag: md5sum.digest('hex') }
    }.bind(this))
  }
 ...
 *
 * FIXME: the root should be set on line 1 of app.js, before the app gets assembled, so that Sara can correctly do things in production/development differences
 */
Sara.prototype.init = function (options, fn) {
  this.use(require('sara-request'))
...
Sara.prototype.use = function (middleware) {
  if (IS_SERVER) {
    middleware(function server(name, mw) {      
      if (this[name] !== undefined) console.warn('Overwrote Sara native module with middleware \'' + name + '\'!')
      this[name] = mw
      this[name].__middleware__ = true
    }.bind(this), function client(name, mw) { /* noop */ })
  } else if (IS_CLIENT) {
    middleware(function server(name, mw) {
      // noop
    }, function client(name, mw) {
      if (this[name] !== undefined) console.warn('Overwrote Sara native module with middleware \'' + name + '\'!')
      this[name] = function (arg, callback) {
        mw.apply(null, arguments)

        // setTimeout(function () {
        //   var id = Date.now()
        //   this.tasks[id] = callback
        //   this.socket.send('mw:' + id + ':' + name + ':' + encodeURIComponent(arg))
        // }.bind(this))
      }
      this[name].__middleware__ = true
    }.bind(this))
...
Sara.prototype.filter = function (pre, ext) {
  this.filters[ext] = this.filters[ext] || []
  this.filters[ext].push(pre)
  return this
}
